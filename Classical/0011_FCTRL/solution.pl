...
# term gave the name to the cellular phone) and every phone connects
# to the BTS with the strongest signal (in a little simplified view).
# Of course, BTSes need some attention and technicians need to check
# their function periodically.
# 
...
# building. Programmers have spent several months studying this problem
# but with no results. They were unable to find the solution fast
# enough. After a long time, one of the programmers found this problem
# in a conference article. Unfortunately, he found that the problem
# is so called "Travelling Salesman Problem" and it is very hard to
# solve. If we have N BTSes to be visited, we can visit them in any
# order, giving us N! possibilities to examine. The function expressing
...
# never "lose" any trailing zero by multiplying by any positive number.
# We can only get new and new zeros. The function Z is very interesting,
...
# follow. Then there are T lines, each containing exactly one positive
...
while (my $number = <>) {
    my  $zeros  = 0;
        $zeros += $number = int ($number / 5) while $number;
    say $zeros;
}
